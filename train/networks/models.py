...
class MattingModule(nn.Module):
    ...
    def forward(self, image):#, two_chan_trimap, image_n, trimap_transformed):
        ...
        resnet_input = image
        two_chan_trimap = torch.zeros((image.shape[0], 2, image.shape[2], image.shape[3]), device=image.device)
        conv_out, indices = self.encoder(resnet_input, return_feature_maps=True)
        result = self.decoder(conv_out, image, indices, two_chan_trimap)
...
class fba_decoder(nn.Module):
    def __init__(self, num_output_channels, batch_norm=False):
        ...
        self.conv_up4 = nn.Sequential(
            ...
            nn.LeakyReLU(),
            nn.Conv2d(16, num_output_channels, kernel_size=1, padding=0, bias=True)
        )
